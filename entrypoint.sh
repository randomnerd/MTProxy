...
exec /usr/bin/mtproto-proxy $@ -p 2398 -H 443 --aes-pwd ${REMOTE_SECRET} --user root -M ${WORKERS} --nat-info $INTERNAL_IP:$EXTERNAL_IP ${SECRET_CMD} ${TAG_CMD} --allow-skip-dh --http-stats -C ${MAX_CONNECTIONS} --address 0.0.0.0 ${REMOTE_CONFIG}
